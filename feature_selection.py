...
def complete_feature_selection(X_train_num,X_test_num,y_train):
    try:
        ...
        logger.info(f'coloumn we need to remove from quansi constant technique:{X_train_num.columns[~reg_con.get_support()]} ')
        ...
        logger.info(f'coloumn we need to remove from constant technique:{X_train_num_fs.columns[~reg_quasi.get_support()]} ')
        ...
        X_train_num_fs_1 = X_train_num_fs_1.drop(['DebtRatioyeo_trim'],axis=1)
        X_test_num_fs_2 = X_test_num_fs_2.drop(['DebtRatioyeo_trim'], axis=1)
        logger.info(f"{X_train_num_fs_1.columns}->{X_train_num_fs_1.shape}")
        logger.info(f"{X_test_num_fs_2.columns}->{X_test_num_fs_2.shape}")
