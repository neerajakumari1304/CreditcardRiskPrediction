...
def random_sample_imputation_technique(X_train,X_test):
    try:
        ...
        logger.info(f"Before technique : {X_train.columns}")
        logger.info(f"Before technique : {X_test.columns}")
        logger.info(f"Before technique : {X_train.isnull().sum()}")
        logger.info(f"Before technique : {X_test.isnull().sum()}")
        for i in X_train.columns:
           if X_train[i].isnull().sum() >0:
               logger.info(f"Train coloumn name:{i}")
        ...
        logger.info(f"After technique : {X_train.columns}")
        logger.info(f"After technique : {X_test.columns}")
        logger.info(f"After technique : {X_train.isnull().sum()}")
        logger.info(f"After technique : {X_test.isnull().sum()}")
        logger.info(f'total rows in training data: {X_train.shape}')
        logger.info(f'total rows in testing data: {X_test.shape}')
